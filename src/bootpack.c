...
void MilfaMain(void)
{
    ...
    {
        memset(infoLayer->buffer, 14, infoLayer->width * infoLayer->height);
        char s[256];
    ...
    {
        memset(keyboardInfoLayer->buffer, 15, keyboardInfoLayer->width * keyboardInfoLayer->height);
        layer_refresh_entire(layerControl, keyboardInfoLayer);
    }

    struct Layer* mouseInfoLayer = layer_create(layerControl, 0, 20, 300, 20);
    {
        memset(mouseInfoLayer->buffer, 15, mouseInfoLayer->width * mouseInfoLayer->height);
        layer_refresh_entire(layerControl, mouseInfoLayer);
    }

    int mouse_x = 16, mouse_y = 16;
    struct Layer* mouseLayer = layer_create(layerControl, 100, 100, 16, 16);
    {
        memset(mouseLayer->buffer, 1, mouseLayer->width * mouseLayer->height);
